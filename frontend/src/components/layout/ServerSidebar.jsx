...
export const ServerSidebar = ({
    ...
    return (
        <div className="w-60 bg-black/20 backdrop-blur-sm border-r border-white/5 flex flex-col h-full">
            {/* Server Header Dropdown */}
            <div
                className="h-12 border-b border-white/5 flex items-center px-4 font-bold text-white hover:bg-white/5 cursor-pointer transition-colors shadow-sm justify-between relative shrink-0"
            ...
            >
                ...
                {showDropdown && (
                    <div className="absolute top-12 left-0 w-full bg-[#18191c] rounded p-1.5 shadow-xl z-50 animate-in fade-in zoom-in-95 duration-100 border border-white/10">
                        <div className="flex items-center justify-between text-[#949cf7] hover:bg-[#4752c4] hover:text-white px-2 py-2 rounded pointer-events-auto cursor-pointer"
                            onClick={(e) => { e.stopPropagation(); onOpenInvite(); setShowDropdown(false); }}>
                            <span>Invite People</span><UserPlus className="w-4 h-4" />
                        </div>
                        {isOwner ? (
                            <>
                                <div className="flex items-center justify-between text-gray-400 hover:bg-[#4752c4] hover:text-white px-2 py-2 rounded pointer-events-auto cursor-pointer"
                                ...
                                <div className="flex items-center justify-between text-red-500 hover:bg-red-500 hover:text-white px-2 py-2 rounded pointer-events-auto cursor-pointer"
                            ...
                            <div className="space-y-[1px]">
                                {categoryChannels.map(channel => (
                                    <div
                                        ...
                                        className={`flex items-center justify-between px-2 py-1.5 rounded cursor-pointer group transition-all ${selectedChannel?._id === channel._id ? 'bg-white/10 text-white' : 'text-gray-400 hover:bg-white/5 hover:text-gray-200'
                                            }`}
                                    >
